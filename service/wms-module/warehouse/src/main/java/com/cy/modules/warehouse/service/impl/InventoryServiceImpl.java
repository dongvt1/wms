...
public class InventoryServiceImpl extends ServiceImpl<InventoryMapper, Inventory> implements InventoryService {
    ...
    public String adjustInventory(String productId, Integer delta, String reason) {
        try {
            Inventory inventory = getByProductId(productId);
            if (inventory == null) {
                return "调整失败: 未找到产品库存记录 " + productId;
            }

            int newQuantity = inventory.getQuantity() + delta;
            if (newQuantity < 0) {
                return "调整失败: 库存不足，当前库存为 " + inventory.getQuantity() + "，调整量为 " + delta;
            }
            ...
            if (updated) {
                log.info("库存调整成功，产品ID: {}, 调整量: {}, 原因: {}", productId, delta, reason);
                return "库存调整成功";
            } else {
                return "调整失败: 更新库存记录失败";
            }
        } catch (Exception e) {
            log.error("调整库存失败，产品ID: {}", productId, e);
