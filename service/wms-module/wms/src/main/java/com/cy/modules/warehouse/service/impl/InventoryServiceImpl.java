...
import com.cy.modules.warehouse.mapper.InventoryMapper;
import com.cy.modules.warehouse.service.InventoryService;
import com.cy.modules.warehouse.service.InventoryTransactionService;
import com.cy.modules.warehouse.service.InventoryAdjustmentService;
...
public class InventoryServiceImpl extends ServiceImpl<InventoryMapper, Inventory> implements InventoryService {
    ...
    @Override
    @Transactional
    public boolean updateInventoryQuantity(String productId, Integer quantity, Integer reservedQuantity,Integer availableQuantity, String updatedBy) {
        return baseMapper.updateQuantity(productId, quantity, reservedQuantity, availableQuantity,updatedBy) > 0;
    }
    ...
    @Override
    @Transactional
    public String adjustInventory(String productId, Integer delta, String reason) {
        try {
            Inventory inventory = getByProductId(productId);
            if (inventory == null) {
                return "Adjustment failed: inventory record not found for product " + productId;
            }

            int newQuantity = inventory.getQuantity() + delta;
            if (newQuantity < 0) {
                return "Adjustment failed: insufficient stock, current quantity is " + inventory.getQuantity() + ", adjustment is " + delta;
            }

            inventory.setQuantity(newQuantity);
            inventory.setAvailableQuantity(calculateAvailableQuantity(newQuantity, inventory.getReservedQuantity()));
            inventory.setLastUpdated(new Date());
            inventory.setUpdatedBy(reason);

            boolean updated = this.updateById(inventory);
            if (updated) {
                log.info("Inventory adjusted successfully, product ID: {}, delta: {}, reason: {}", productId, delta, reason);
                return "Inventory adjusted successfully";
            } else {
                return "Adjustment failed: failed to update inventory record";
            }
        } catch (Exception e) {
            log.error("Failed to adjust inventory, product ID: {}", productId, e);
            return "Adjustment failed: " + e.getMessage();
        }
    }
