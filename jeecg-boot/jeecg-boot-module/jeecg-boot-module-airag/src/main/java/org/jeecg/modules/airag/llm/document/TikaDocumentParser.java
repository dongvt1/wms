...
/**
 * tikaDocument parser,rewritelangchain4jofTikaDocumentParser <br/>
 * jeecgbootNot currently supportedpoi5.x,solangchain4j同of方法不能用,Realize it yourself
 * @author chenrui
 * @date 2025/3/5 16:19
 */
public class TikaDocumentParser {
    ...
    public Document parse(File file) {
        AssertUtils.assertNotEmpty("Please select file", file);
        try {
            // for parsing
            InputStream isForParsing = Files.newInputStream(file.toPath());
            // use Tika Automatic detection MIME type
            ...
            if (fileName.endsWith(".txt")
                ...
                throw new IllegalArgumentException("不支持ofdocumentFormat: " + FilenameUtils.getExtension(fileName));
            }
        } catch (IOException e) {
    ...
    private static Document tryExtractDocOrDocx(InputStream inputStream) throws IOException {
        try {
            // try first DOCX（based on OPC XML Format）
            return extractTextFromDocx(inputStream);
        } catch (Exception e1) {
            try {
                // if DOCX Parsing failed，then try DOC（based on二进制Format）
                return extractTextFromDoc(inputStream);
            } catch (Exception e2) {
                throw new IOException("Unable to parse DOC or DOCX document", e2);
            }
        }
    }

    /**
     * usetikaextractdocument内容 <br/>
     * pdf/text/md等documentusetikaextract
     *
     * @param inputStream
     * @return
     * @author chenrui
     ...
     * @author chenrui
     * @date 2025/3/5 14:42
     * @deprecated becausejeecg主项目Not currently supportedpoi5.x, Realize it yourselfextract功能.
     */
    @Deprecated
