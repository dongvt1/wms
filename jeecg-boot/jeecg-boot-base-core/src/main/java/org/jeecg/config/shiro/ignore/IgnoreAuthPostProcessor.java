...
/**
 * existspring bootduring initialization，according to@RestControllerAnnotation gets currentspringin containerbean
 * @author eightmonth
...
public class IgnoreAuthPostProcessor implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        log.info("Init Token ignoreAuthUrls Config [ gather ]  ：{}", ignoreAuthUrls);
        if (!CollectionUtils.isEmpty(ignoreAuthUrls)) {
            InMemoryIgnoreAuth.set(ignoreAuthUrls);
        }

        // Calculation time of method
        long endTime = System.currentTimeMillis();
        long elapsedTime = endTime - startTime;
        log.info("Init Token ignoreAuthUrls Config [ time consuming ] ：" + elapsedTime + "millisecond");
    }
    ...
    private List<String> rebuildUrl(String[] bases, String[] uris) {
        ...
        if (bases.length > 0) {
            for (String base : bases) {
                for (String uri : uris) {
                    // ifuriContains path placeholders, then you need to replace it with*
                    if (uri.matches(".*\\{.*}.*")) {
                        uri = uri.replaceAll("\\{.*?}", "*");
                    }
