...
INSERT INTO "NACOS"."CONFIG_INFO"("ID","DATA_ID","GROUP_ID","CONTENT","MD5","GMT_CREATE","GMT_MODIFIED","SRC_USER","SRC_IP","APP_NAME","TENANT_ID","C_DESC","C_USE","EFFECT","TYPE","C_SCHEMA","ENCRYPTED_DATA_KEY") VALUES(2,'jeecg-dev.yaml','DEFAULT_GROUP','spring:
  datasource:
    druid:
      ...
      web-stat-filter:
        enabled: true
    dynamic:
      druid: # overall situationdruidparameter，Most values ​​remain the same as the default。(现已支持的parameter如下,If you don’t know the meaning, don’t set it randomly.)
        # Connection pool configuration information
        # initial size，smallest，maximum
        initial-size: 5
        min-idle: 5
        maxActive: 20
        # Configure the timeout to wait for a connection
        maxWait: 60000
        # Configure how often the test should be performed，Detect idle connections that need to be closed，The unit is milliseconds
        timeBetweenEvictionRunsMillis: 60000
        # Configuration一个连接在池中smallest生存的时间，The unit is milliseconds
        minEvictableIdleTimeMillis: 300000
        validationQuery: SELECT 1 FROM DUAL
        testWhileIdle: true
        testOnBorrow: false
        testOnReturn: false
        # OpenPSCache，and specify on each connectionPSCachesize
        poolPreparedStatements: true
        maxPoolPreparedStatementPerConnectionSize: 20
        # Configure monitoring statistics interceptionfilters，Remove the rear monitoring interfacesqlUnable to count，''wall''for firewall
  ...
  redis:
    ...
    lettuce:
      pool:
        max-active: 8   #maximum连接database连接数,set up 0 for no limit
        max-idle: 8     #maximumwait待连接中的数量,set up 0 for no limit
        max-wait: -1ms  #maximum建立连接wait待时间。If this time is exceeded, an exception will be received。set up为-1means unlimited。
        min-idle: 0     #smallestwait待连接中的数量,set up 0 for no limit
  ...
  data:
    mongodb:
      #Password connection If the account password contains special characters, you need to useURLEncodercoding Library name is required
      ...
      print: true  #Whether to print query statements
      slowQuery: true  #Whether to record slow queries into the database
      slowTime: 1000 #Slow query minimum time,Default is1000millisecond
...
jeecg:
  firewall:
    ...
    lowCodeMode: dev
  # Signing key string(The front and back ends must be consistent，Please modify it yourself for official release.)
  ...
  path :
    #File upload root directory set up置
    upload: /opt/upFiles
    #webappfile path
    webapp: /opt/webapp
  shiro:
    excludeUrls: /test/jeecgDemo/demo3,/test/jeecgDemo/redisDemo/**,/bigscreen/category/**,/bigscreen/visual/**,/bigscreen/map/**,/jmreport/bigscreen2/**
  #Alibaba Cloudoss存储Configuration
  oss:
    ...
    bucketName: jeecgdev
    staticDomain: ??
  # ElasticSearch 6set up置
  elasticsearch:
    cluster-name: jeecg-ES
    cluster-nodes: 127.0.0.1:9200
    check-enabled: false
  # 表单set up计器Configuration
  desform:
    # theme color（Only supports 16hexadecimal color code）
    theme-color: "#1890ff"
    # document、Image upload method，Optional：qiniu（Qiniuyun）、system（跟随系统Configuration）
    upload-type: system
    map:
      # Configuration百度地图的AK，Application address：https://lbs.baidu.com/apiconsole/key?application=key#/home
      baidu: ??
  # 在线预览document服务器地址Configuration
  ...
  ai-chat:
    ...
    timeout: 60
  #WpsOnline documentation
  wps:
    domain: https://wwo.wps.cn/office/
    appid: ??
    appsecret: ??
  #xxl-jobConfiguration
  xxljob:
    enabled: false
...
third-app:
  ...
  type:
    ...
    WECHAT_ENTERPRISE:
      ...
      client-secret: ??
      agent-id: ??
      #Create your own application key（新版企微需要Configuration）
      # agent-app-secret: ??
    #DingTalk
    DINGTALK:
      enabled: false
...
management:
  ...
  endpoints:
    web:
      exposure:
        include: "*" #Expose all nodes
    health:
      sensitive: true #Turn off filtering sensitive information
  endpoint:
    health:
      show-details: ALWAYS  #Show details
flowable:
  # 自动部署验证set up置:true-turn on（default）、false-closure
  check-process-definitions: false
  #Configuration项可以set up置流程引擎启动和closure时database执行的策略
  database-schema-update: false
  #保存历史数据级别set up置为fullhighest level，Facilitates tracing of historical data
  history-level: full
  #turn onscheduled tasks
...
spring:
      ...
      mail:
        smtp:
          ...
          starttls:
            enable: true
            required: true
  ## quartzscheduled tasks,Using database method
  quartz:
    job-store-type: jdbc
    initialize-schema: embedded
    #set up置自动启动，Default is true
    auto-startup: false
    #Delay1秒启动scheduled tasks
    startup-delay: 1s
    #Update existing ones at startupJob
  ...
  # set up置静态file path，js,csswait
  mvc:
    static-path-pattern: /**
    #Spring Boot 2.6+ manually specified asant-path-matcher
    pathmatch:
...
#mybatis plus set up置
mybatis-plus:
  mapper-locations: classpath*:org/jeecg/**/xml/*Mapper.xml
  global-config:
    # closureMP3.0Comes with itbanner
    banner: false
    db-config:
      #Primary key type  0:"databaseIDself-increasing",1:"该类型为未set up置Primary key type", 2:"user inputID",3:"overall situation唯一ID (Number type is uniqueID)", 4:"overall situation唯一ID UUID",5:"字符串overall situation唯一ID (idWorker string representation of)";
      id-type: ASSIGN_ID
      # defaultdatabase表下划线命名
      table-underline: true
  configuration:
    # 这个Configuration会将执行的sqlPrint it out，Can be used during development or testing
    #log-impl: org.apache.ibatis.logging.stdout.StdOutImpl
    # The return type isMap,shownullCorresponding fields
...
INSERT INTO "NACOS"."CONFIG_INFO"("ID","DATA_ID","GROUP_ID","CONTENT","MD5","GMT_CREATE","GMT_MODIFIED","SRC_USER","SRC_IP","APP_NAME","TENANT_ID","C_DESC","C_USE","EFFECT","TYPE","C_SCHEMA","ENCRYPTED_DATA_KEY") VALUES(5,'jeecg-gateway-dev.yaml','DEFAULT_GROUP','jeecg:
  route:
    config:
      #Route loading mode： database、nacos、yml
      data-type: database
      #Nacosmodel，读取Configurationdocumentjeecg-gateway-router.json(fixed)
      group: DEFAULT_GROUP
...
INSERT INTO "NACOS"."TENANT_INFO"("ID","KP","TENANT_ID","TENANT_NAME","TENANT_DESC","CREATE_SOURCE","GMT_CREATE","GMT_MODIFIED") VALUES(1,'1','springboot3','springboot3','springboot3版本Configurationdocument，andspringboot2There is a big difference','nacos',1720506551826,1720506551826);
...
COMMENT ON COLUMN "NACOS"."CONFIG_INFO"."GMT_CREATE" IS 'creation time';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO"."GMT_MODIFIED" IS 'modification time';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO"."SRC_USER" IS 'source user';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO"."SRC_IP" IS 'source ip';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO"."TENANT_ID" IS 'Tenant fields';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO"."ENCRYPTED_DATA_KEY" IS 'key';

COMMENT ON TABLE "NACOS"."CONFIG_INFO_AGGR" IS 'Add tenant field';
...
COMMENT ON COLUMN "NACOS"."CONFIG_INFO_AGGR"."CONTENT" IS 'content';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO_AGGR"."GMT_MODIFIED" IS 'modification time';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO_AGGR"."TENANT_ID" IS 'Tenant fields';
...
COMMENT ON COLUMN "NACOS"."CONFIG_INFO_BETA"."GMT_CREATE" IS 'creation time';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO_BETA"."GMT_MODIFIED" IS 'modification time';
...
COMMENT ON COLUMN "NACOS"."CONFIG_INFO_BETA"."TENANT_ID" IS 'Tenant fields';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO_BETA"."ENCRYPTED_DATA_KEY" IS 'key';
...
COMMENT ON COLUMN "NACOS"."CONFIG_INFO_TAG"."GMT_CREATE" IS 'creation time';

COMMENT ON COLUMN "NACOS"."CONFIG_INFO_TAG"."GMT_MODIFIED" IS 'modification time';
...
COMMENT ON TABLE "NACOS"."GROUP_CAPACITY" IS 'cluster、eachGroupCapacity information table';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."ID" IS 'primary keyID';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."GROUP_ID" IS 'Group ID，空字符表示整个cluster';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."QUOTA" IS 'quota，0表示使用default值';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."USAGE" IS 'Usage';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."MAX_SIZE" IS '单个Configuration大小上限，Unit is bytes，0表示使用default值';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."MAX_AGGR_COUNT" IS '聚合子Configurationmaximum个数，，0表示使用default值';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."MAX_AGGR_SIZE" IS '单个聚合数据的子Configuration大小上限，Unit is bytes，0表示使用default值';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."MAX_HISTORY_COUNT" IS 'maximum变更历史数量';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."GMT_CREATE" IS 'creation time';

COMMENT ON COLUMN "NACOS"."GROUP_CAPACITY"."GMT_MODIFIED" IS 'modification time';

COMMENT ON TABLE "NACOS"."HIS_CONFIG_INFO" IS 'Multi-tenant renovation';

COMMENT ON COLUMN "NACOS"."HIS_CONFIG_INFO"."APP_NAME" IS 'app_name';

COMMENT ON COLUMN "NACOS"."HIS_CONFIG_INFO"."TENANT_ID" IS 'Tenant fields';

COMMENT ON COLUMN "NACOS"."HIS_CONFIG_INFO"."ENCRYPTED_DATA_KEY" IS 'key';

COMMENT ON TABLE "NACOS"."TENANT_CAPACITY" IS '租户Capacity information table';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."ID" IS 'primary keyID';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."TENANT_ID" IS 'Tenant ID';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."QUOTA" IS 'quota，0表示使用default值';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."USAGE" IS 'Usage';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."MAX_SIZE" IS '单个Configuration大小上限，Unit is bytes，0表示使用default值';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."MAX_AGGR_COUNT" IS '聚合子Configurationmaximum个数';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."MAX_AGGR_SIZE" IS '单个聚合数据的子Configuration大小上限，Unit is bytes，0表示使用default值';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."MAX_HISTORY_COUNT" IS 'maximum变更历史数量';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."GMT_CREATE" IS 'creation time';

COMMENT ON COLUMN "NACOS"."TENANT_CAPACITY"."GMT_MODIFIED" IS 'modification time';
...
COMMENT ON COLUMN "NACOS"."TENANT_INFO"."GMT_CREATE" IS 'creation time';

COMMENT ON COLUMN "NACOS"."TENANT_INFO"."GMT_MODIFIED" IS 'modification time';
