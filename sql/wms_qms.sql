...
CREATE TABLE IF NOT EXISTS `wh_qms_checklist_template` (
...
CREATE TABLE IF NOT EXISTS `wh_qms_checklist_item` (
    `id`               VARCHAR(36)   NOT NULL COMMENT 'ID',
    `template_id`      VARCHAR(36)   NOT NULL COMMENT 'FK → wh_qms_checklist_template',
...
CREATE TABLE IF NOT EXISTS `wh_iqc_inspection` (
    `id`                    VARCHAR(36)    NOT NULL COMMENT 'ID',
    ...
    `template_id`           VARCHAR(36)    NULL     COMMENT 'FK → wh_qms_checklist_template',
    ...
    `status`                VARCHAR(20)    NOT NULL DEFAULT 'draft' COMMENT 'draft, in_progress, passed, failed, conditional',
    `notes`                 TEXT           NULL,
    `create_by`             VARCHAR(50)    NULL,
    `create_time`           DATETIME       NULL,
    `update_by`             VARCHAR(50)    NULL,
...
CREATE TABLE IF NOT EXISTS `wh_iqc_inspection_result` (
    `id`                 VARCHAR(36)   NOT NULL COMMENT 'ID',
    `inspection_id`      VARCHAR(36)   NOT NULL COMMENT 'FK → wh_iqc_inspection',
    `checklist_item_id`  VARCHAR(36)   NULL     COMMENT 'FK → wh_qms_checklist_item',
...
CREATE TABLE IF NOT EXISTS `wh_pqc_inspection` (
    `id`                 VARCHAR(36)    NOT NULL COMMENT 'ID',
    ...
    `template_id`        VARCHAR(36)    NULL     COMMENT 'FK → wh_qms_checklist_template',
    `stage_id`           VARCHAR(36)    NULL     COMMENT 'FK → wh_production_stage (tùy chọn)',
    ...
    `status`             VARCHAR(20)    NOT NULL DEFAULT 'draft' COMMENT 'draft, in_progress, passed, failed',
    `notes`              TEXT           NULL,
    `create_by`          VARCHAR(50)    NULL,
    `create_time`        DATETIME       NULL,
    `update_by`          VARCHAR(50)    NULL,
...
CREATE TABLE IF NOT EXISTS `wh_pqc_inspection_result` (
    `id`                 VARCHAR(36)   NOT NULL COMMENT 'ID',
    `inspection_id`      VARCHAR(36)   NOT NULL COMMENT 'FK → wh_pqc_inspection',
    `checklist_item_id`  VARCHAR(36)   NULL     COMMENT 'FK → wh_qms_checklist_item',
    `criterion_name`     VARCHAR(200)  NOT NULL COMMENT 'Tên tiêu chí (copy)',
    `standard_value`     VARCHAR(200)  NULL     COMMENT 'Giá trị tiêu chuẩn (copy)',
    `actual_value`       VARCHAR(500)  NULL     COMMENT 'Giá trị thực đo',
