<template>
    <BasicModal v-bind="$attrs" @register="registerModal" title="Chi tiết phiếu PQC" width="860px" :footer="null">
        <template v-if="detail">
            <a-descriptions bordered size="small" :column="2">
                ...
                <a-descriptions-item label="Lệnh SX">{{ detail.inspection?.workOrderId }}</a-descriptions-item>
                <a-descriptions-item label="Người KT">{{ detail.inspection?.inspector }}</a-descriptions-item>
                <a-descriptions-item label="SL kiểm tra">{{ detail.inspection?.quantityInspected
                    }}</a-descriptions-item>
...
<script lang="ts" name="pqc-detail-modal" setup>
import { ref } from 'vue';
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (data?.id) {
        const res: any = await pqcApi.queryById(data.id);
        detail.value = res;
    }
});

async function approve(status: string) {
    await pqcApi.approve(detail.value.inspection.id, status);
...
function statusColor(s: string) {
    const m: Record<string, string> = { draft: 'default', in_progress: 'blue', passed: 'green', failed: 'red' };
    return m[s] || 'default';
}
function statusLabel(s: string) {
    const m: Record<string, string> = { draft: 'Nháp', in_progress: 'Đang KT', passed: 'Đạt', failed: 'Không đạt' };
    return m[s] || s;
}
</script>
