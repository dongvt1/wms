<template>
  <div :class="prefixCls" class="login-background-img">
            ...
            <div class="aui-formBox">
              ...
              <a-form @keyup.enter.native="loginHandleClick">
                ...
                <div class="aui-flex" :class="`${prefixCls}-sign-in-way`">
                  ...
                  <div class="aui-flex-box">
                    <div class="aui-third-login">
                      <a title="Enterprise WeChat" @click="onThirdLogin('wechat_enterprise')"><icon-font class="item-icon" type="icon-qiyeweixin3" /></a>
                    </div>
                  </div>
                  <div class="aui-flex-box">
                    <div class="aui-third-login">
                      <a title="DingTalk" @click="onThirdLogin('dingtalk')"><DingtalkCircleFilled /></a>
                    </div>
                  </div>
                  <div class="aui-flex-box">
                    <div class="aui-third-login">
                      <a title="WeChat" @click="onThirdLogin('wechat_open')"><WechatFilled /></a>
                    </div>
                  </div>
                </div>
              </a-form>
    ...
    <!-- Third-party login related pop-up boxes -->
    <ThirdModal ref="thirdModalRef"></ThirdModal>
    
    <!-- Image verification code pop-up window -->
...
<script lang="ts" setup name="login-mini">
  ...
  //Mobile phone number login or account login?
  const activeIndex = ref<string>('accountLogin');
  const type = ref<string>('login');
  //Account login form fields
  ...
  const loginRef = ref();
  //Third-party login pop-up window
  const thirdModalRef = ref();
  //Scan the QR code to log in
  const codeRef = ref();
  //Whether to display Get Verification Code
  ...
  function handleChangeCheckCode() {
    formData.inputCode = '';
    //update-begin---author:chenrui ---date:2025/1/7  for：[QQYUN-10775]Verification codes can be reused #7674------------
    randCodeData.checkKey = new Date().getTime() + Math.random().toString(36).slice(-4); // 1629428467008;
    //update-end---author:chenrui ---date:2025/1/7  for：[QQYUN-10775]Verification codes can be reused #7674------------
  ...
  /**
   * Log in with account or mobile phone
   */
  ...
  async function getLoginCode() {
    ...
    //update-begin---author:wangshuai---date:2025-07-15---for:【issues/8567】serious：There is a horizontal override problem when changing passwords：登录应该用登录模板不应该用forget the password的模板---
    const result = await getCaptcha({ mobile: phoneFormData.mobile, smsmode: SmsEnum.LOGIN }).catch((res) =>{
    //update-end---author:wangshuai---date:2025-07-15---for:【issues/8567】serious：There is a horizontal override problem when changing passwords：登录应该用登录模板不应该用forget the password的模板---
  ...
  /**
   * forget the password/register账号回调事件
   * @param value
   */
...
html[data-theme='dark'] {
  .@{prefix-cls} {
    ...
    //update-begin---author:wangshuai ---date:20230828  for：【QQYUN-6363】There is something wrong with this style code，not inside，causing problems with expressions------------
    &-sign-in-way {
      .anticon {
    ...
    //update-end---author:wangshuai ---date:20230828  for：【QQYUN-6363】There is something wrong with this style code，not inside，causing problems with expressions------------
  }
