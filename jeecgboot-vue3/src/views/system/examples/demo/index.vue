  ...
  <div>
        ...
        <a-row :gutter="24">
          <a-col :lg="8">
            <a-form-item label="username">
              <a-input placeholder="Please enter the name for fuzzy query" v-model:value="queryParam.name"></a-input>
            </a-form-item>
          </a-col>
          <a-col :lg="8">
            <a-form-item label="age">
              <a-input placeholder="最小age" type="ge" v-model:value="queryParam.age_begin" style="width: calc(50% - 15px)"></a-input>
              <span>~</span>
              <a-input placeholder="最大age" type="le" v-model:value="queryParam.age_end" style="width: calc(50% - 15px)"></a-input>
            </a-form-item>
          </a-col>
          <template v-if="toggleSearchStatus">
            <a-col :lg="8">
              <a-form-item label="gender">
                <JDictSelectTag v-model:value="queryParam.sex" placeholder="请选择gender" dictCode="sex" />
              </a-form-item>
            </a-col>
            <a-col :lg="8">
              <a-form-item label="Select user">
                <JDictSelectTag v-model:value="queryParam.id" placeholder="请Select user" dictCode="demo,name,id" />
              </a-form-item>
            </a-col>
          </template>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :class="{ 'p-4': customSearch }">
      <template #form-age="{ model, field }">
        <a-input placeholder="最小age" type="ge" v-model:value="min" style="width: calc(50% - 15px)" @change="ageChange(model, field)"></a-input>
        <span>~</span>
        <a-input placeholder="最大age" type="le" v-model:value="max" style="width: calc(50% - 15px)" @change="ageChange(model, field)"></a-input>
      </template>
      <template #tableTitle>
        ...
        <a-button preIcon="ant-design:export-outlined" type="primary" @click="handleExportXls('Single table example', getExportUrl,exportParams)">Export</a-button>
        <a-button preIcon="ant-design:filter" type="primary" @click="">高级Query</a-button>
        <a-button preIcon="ant-design:plus-outlined" type="primary" @click="openTab">OpenTabPage</a-button>
        <a-button preIcon="ant-design:retweet-outlined" type="primary" @click="customSearch = !customSearch">{{
          customSearch ? '表单配置Query' : '自定义Query'
        }}</a-button>
        <a-button preIcon="ant-design:import-outlined" type="primary" @click="handleImport">弹窗import</a-button>
...
<script lang="ts" setup>
  ...
  const [registerTable, { reload, setProps }] = useTable({
    ...
    //Customize default multi-field sorting
    defSort: [
      {
        column: 'name',
        order: 'desc',
