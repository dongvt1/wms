  ...
  <div v-if="text != ''" class="textWrap" :class="[inversion === 'user' ? 'self' : 'chatgpt']" ref="textRef">
    <div v-if="inversion != 'user'" :style="{ width: getIsMobile? screenWidth : 'auto' }">
      ...
      <template v-if="showRefKnow">
        <a-divider orientation="left">References</a-divider>
...
<script setup lang="ts">
  ...
  // Whether to display reference knowledge base
  ...
  const replaceImageWith = markdownContent => {
    // Support picture settingswidthHow to write ![](/static/jimuImages/screenshot_1617252560523.png =100)
    const regex = /!\[([^\]]*)\]\(([^)]+)=([0-9]+)\)/g;
  ...
  //Whether to enlarge the picture
  ...
  /**
   * set upmarkdown bodyoverall width
   */
