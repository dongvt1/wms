    ...
    <BasicModal destroyOnClose @register="registerModal" :canFullscreen="false" width="600px" :title="title" @ok="handleOk" @cancel="handleCancel">
      <div class="flex header">
        <a-input
          ...
          placeholder="Enter flow name and press Enter to search"
        ></a-input>
      </div>
      <a-row :span="24">
      ...
      <div v-if="flowId" class="use-select">
        Selected <span class="ellipsis" style="max-width: 150px">{{flowData.name}}</span>
...
<script lang="ts">
  ...
  export default {
    ...
    setup(props, { emit }) {
      const title = ref<string>('Select Flow');
      //Application type
