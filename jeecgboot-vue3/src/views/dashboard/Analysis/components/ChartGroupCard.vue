<template>
  <div class="md:flex">
    <template v-for="(item, index) in dataList" :key="item.title">
      ...
      >
        <template #action>
          <a-tooltip title="Indicator description">
            <Icon :icon="item.icon" :size="20" />
          </a-tooltip>
        </template>
        <div v-if="type === 'chart'">
          <Trend term="Weekly year-on-year" :percentage="12" v-if="index === 0" />
          <Trend term="Year-on-year" :percentage="11" v-if="index === 0" :type="false" />
        ...
        <template #footer v-if="type === 'chart'">
          ...
          >
          <Trend term="Weekly year-on-year" :percentage="12" v-if="index === 3" />
          <Trend term="Year-on-year" :percentage="11" v-if="index === 3" :type="false" />
        </template>
