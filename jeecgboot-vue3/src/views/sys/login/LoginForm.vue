<template>
  ...
  <Form class="p-4 enter-x" :model="formData" :rules="getFormRules" ref="formRef" v-show="getShow" @keypress.enter="handleLogin">
    ...
    <div class="flex justify-evenly enter-x" :class="`${prefixCls}-sign-in-way`">
      ...
      <a @click="onThirdLogin('wechat_enterprise')" title="Enterprise WeChat"> <icon-font class="item-icon" type="icon-qiyeweixin3" /></a>
      <a @click="onThirdLogin('dingtalk')" title="DingTalk"><DingtalkCircleFilled /></a>
      <a @click="onThirdLogin('wechat_open')" title="WeChat"><WechatFilled /></a>
    </div>
  </Form>
...
<script lang="ts" setup>
  ...
  async function handleLogin() {
    ...
    try {
      ...
      //update-begin-author:taoyan date:2022-5-3 for: issues/41 Login page，当输入Verification code错误时，Verification code图片要刷新一下，而不是保持旧的Verification code图片不变
      handleChangeCheckCode();
      //update-end-author:taoyan date:2022-5-3 for: issues/41 Login page，当输入Verification code错误时，Verification code图片要刷新一下，而不是保持旧的Verification code图片不变
    }
  }
  function handleChangeCheckCode() {
    formData.inputCode = '';
    //TODO compatiblemockand interface，Deal with it like this for now
