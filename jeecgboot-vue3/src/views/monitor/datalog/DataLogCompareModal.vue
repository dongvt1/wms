...
<template>
  <div>
    <BasicModal v-bind="$attrs" @register="register" title="Data comparison" width="50%" destroyOnClose :showOkBtn="false">
      <a-row :gutter="6" v-if="dataVersionList" style="margin-left: 2px">
        <span style="margin-top: 5px; margin-right: 3px; margin-left: 4px">Version comparison:</span>
        <a-select placeholder="version number" @change="handleChange1" v-model:value="params.dataId1">
          <a-select-option v-for="(log, logindex) in dataVersionList" :key="log.value" :value="log.value">
            {{ log.text }}
          </a-select-option>
        </a-select>

        <a-select placeholder="version number" @change="handleChange2" style="padding-left: 10px" v-model:value="params.dataId2">
          <a-select-option v-for="(log, logindex) in dataVersionList" :key="log.value" :value="log.value">
            {{ log.text }}
          </a-select-option>
        </a-select>
      ...
      <BasicTable
        ...
        v-if="isUpdate"
      >
        <template #dataVersionTitle1="{ record }"> <Icon icon="icon-park-outline:grinning-face" /> Version:{{ dataVersion1Num }} </template>
        <template #dataVersionTitle2="{ record }"> <Icon icon="icon-park-outline:grinning-face" /> Version:{{ dataVersion2Num }} </template>
    ...
    setup(props, { emit, refs }) {
      ...
      function handleChange1(value) {
        if (params.dataId2 == value) {
          createMessage.warning('相同version number不能比较');
          return;
        }
