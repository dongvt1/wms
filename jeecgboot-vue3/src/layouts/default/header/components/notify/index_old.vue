      ...
      <template #content>
        <Tabs>
          <template v-for="item in listData" :key="item.key">
            <TabPane>
              ...
              <!-- bindingtitle-clickThe event notification list is titled“Clickable”of-->
        ...
        <a-row class="bottom-buttons">
          <a-col :span="count === 0 ? 0 : 12">
            <a-button @click="onEmptyNotify" type="dashed" block>Clear messages</a-button>
          </a-col>
          <a-col :span="count === 0 ? 24 : 12">
            <a-button @click="popoverVisible = false" type="dashed" block>
              <router-link to="/monitor/mynews">View more</router-link>
            </a-button>
          </a-col>
        </a-row>
      </template>
    ...
    setup() {
      ...
      async function loadData() {
        try {
          ...
          console.warn('System message notification exception：', e);
        }
      }
      ...
      function initWebSocket() {
        ...
        let token = getToken();
        // WebSocket与普通of请求所用协议有所不同，wsEquivalent tohttp，wssEquivalent tohttps
      ...
      function onWebSocketMessage(data) {
        ...
        if (data.cmd === 'topic' || data.cmd === 'user') {
          //update-begin-author:taoyan date:2022-7-13 for: VUEN-1674【seriousbug】System notification，Why do I have to refresh the upper right corner before prompting?
          //Saving data in the background is too slow Front-end delayed refresh message
          setTimeout(()=>{
            loadData();
          }, 1000)
          //update-end-author:taoyan date:2022-7-13 for: VUEN-1674【seriousbug】System notification，Why do I have to refresh the upper right corner before prompting?
        }
      }
