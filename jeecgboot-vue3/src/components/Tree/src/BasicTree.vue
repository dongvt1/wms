<script lang="tsx">
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots, emit, expose }) {
       ...
       * 2024-04-26
       * liaozhiyang
       * 【issues/1151】When the hierarchy is independent, the parent is checked，Then click on the hierarchy to associate the children to visually check it.，But the child level was not saved(Make functions independent and reuse them)
       * */
      const handleCheck = (v: CheckKeys, e?) => {
        let currentValue = toRaw(state.checkedKeys) as KeyType[];
        if (isArray(currentValue) && searchState.startSearch && e) {
          // update-begin-author:liusq---date:20230404--for: [issue/429]Tree search click event fails---
      ...
      watch(
        () => props.value,
        () => {
          // update-end--author:liaozhiyang---date:20231122---for：【issues/863】Close the select department pop-up window，The check before reopening disappeared.
      ...
      // update-begin--author:liaozhiyang---date:20240426---for：【issues/1151】When the hierarchy is independent, the parent is checked，Then click on the hierarchy to associate the children to visually check it.，But the child level was not saved
      watch(
        () => props.checkStrictly,
        () => {
      ...
      // update-end--author:liaozhiyang---date:20240426---for：【issues/1151】When the hierarchy is independent, the parent is checked，Then click on the hierarchy to associate the children to visually check it.，But the child level was not saved
      ...
      return () => {
        ...
        return (
          <div class={[bem(), 'h-full',unref(getBindValues).multiple === false ? 'custom-radio':'' , attrs.class]}>
            ...
            <Spin spinning={unref(props.loading)} tip="loading...">
...
// update-begin--author:liaozhiyang---date:20250908---for：【JHHB-192】Main job selection，Change multi-select box to single-select box
.custom-radio {
  :deep(.ant-tree) {
    .ant-tree-checkbox {
...
// update-end--author:liaozhiyang---date:20250908---for：【JHHB-192】Main job selection，Change multi-select box to single-select box
</style>
