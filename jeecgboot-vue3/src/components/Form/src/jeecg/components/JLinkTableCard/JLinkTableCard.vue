    ...
    <div class="table-link-card">
      <div style="width: 100%; height: 100%">
        <div class="card-button" v-if="showButton">
          <a-button @click="handleAddRecord"><PlusOutlined />remember record</a-button>
        </div>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      // ["表单Field,表字典Field","表单Field,表字典Field"]
      linkFields: propTypes.array.def([]),
      //Is it a disabled page?
      disabled: propTypes.bool.def(false),
      // whether it isdetailpage
    ...
    setup(props, { emit }) {
      ...
      function handleAddRecord(e) {
        prevent(e);
        openListModal(true, {
          // update-begin--author:liaozhiyang---date:20240517---for：【TV360X-43】修复关联rememberrecord可以添加重复数据
          selectedRowKeys: selectRecords.value.map((item) => item.id),
          selectedRows: [...selectRecords.value],
          // update-end--author:liaozhiyang---date:20240517---for：【TV360X-43】修复关联rememberrecord可以添加重复数据
        });
      }
      ...
      watch(
        ...
        async (val) => {
          if (val) {
            ...
            //Ensure that other values ​​in the form are echoed successfully
      ...
      onMounted(() => {
        // update-begin--author:liaozhiyang---date:20240522---for：【TV360X-281】分辨率小时关联rememberrecord文字被图片挤没了
        if (tableLinkCardRef.value.offsetWidth < 250) {
          fixedSpan.value = 24;
        }
        // update-end--author:liaozhiyang---date:20240522---for：【TV360X-281】分辨率小时关联rememberrecord文字被图片挤没了
      });
      // update-begin--author:liaozhiyang---date:20240529---for：【TV360X-389】下拉和卡片关联rememberrecord图裂开给个默认图片
      const handleImageError = (event) => {
        event.target.src = placeholderImage;
      };
      // update-end--author:liaozhiyang---date:20240529---for：【TV360X-389】下拉和卡片关联rememberrecord图裂开给个默认图片
