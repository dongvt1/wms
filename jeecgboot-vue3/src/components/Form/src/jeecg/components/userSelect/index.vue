<template>
  <div>
    <div v-if="isSearchFormComp" @click="click2Add" :class="disabled?'disabled-user-select':''" style="padding:0 5px;background-color: #fff;border: 1px solid #ccc;border-radius: 3px;box-sizing: border-box;display:flex;color: #9e9e9e;font-size: 14px;flex-wrap: wrap;min-height: 32px;">
      ...
      <span v-else style="height: 30px;line-height: 30px;display: inline-block;margin-left: 7px;color: #bfbfbf;">Please select user</span>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    props: {
      ...
      // Whether as a query condition
      query:{
        type: Boolean,
        default: false,
      },
      //How many people can be displayed at most-queryfortrueefficient
      maxCount:{
        type: Number,
        default: 2
      },
      ...
      //是否在高级查询中作for条件 You can select the current user
      inSuperQuery:{
        type: Boolean,
        default: false,
      }
    ...
    setup(props, { emit }) {
      ...
      async function getUserList(ids) {
        ...
        selectedUserList.value = [];
        if(ids){
          // update-begin-author:sunjianlei date:20230330 for: Fixed the problem that the comma-separated echo of the user selector did not take effect.
          ...
          // update-end-author:sunjianlei date:20230330 for: Fixed the problem that the comma-separated echo of the user selector did not take effect.
        }
