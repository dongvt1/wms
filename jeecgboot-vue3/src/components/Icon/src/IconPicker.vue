  ...
  <a-modal :bodyStyle="{ padding: '24px', paddingTop: mode === 'svg' ? '48px' : '24px'}" v-bind="$attrs" v-model:open="iconOpen" :keyboard="false" :width="800" @ok="handleOk" :ok-text="t('common.okText')" :cancel-text="t('common.cancelText')">
    <template v-if="mode === 'iconify'">
      <a-tabs style="padding-left: 15px;padding-right: 15px">
        <a-tab-pane tab="Directional icon" key="1">
          <a-form-item-rest>
        ...
        <a-tab-pane tab="Indicative icons" key="2">
          <a-form-item-rest>
        ...
        <a-tab-pane tab="Edit icon" key="3">
          <a-form-item-rest>
        ...
        <a-tab-pane tab="Data class icon" key="4">
          <a-form-item-rest>
        ...
        <a-tab-pane tab="Common website icons" key="5">
          <a-form-item-rest>
        ...
        <a-tab-pane tab="Branding and Identity" key="6">
          <a-form-item-rest>
...
<script lang="ts" setup name="icon-picker">
  ...
  // Not imported using aliases，It's becauseWebStormThe current version cannot be correctly recognized yet，Reportunusedwarn
  const AInput = Input;
  ...
  //update-begin---author:wangshuai---date:2024-05-08---for:【QQYUN-8924】Icon library selection component is not as good asvue2Convenience---
  ...
  //update-end---author:wangshuai---date:2024-05-08---for:【QQYUN-8924】Icon library selection component is not as good asvue2Convenience---

  watchEffect(() => {
    // update-begin--author:liaozhiyang---date:20240528---for：【TV360X-136】Change button icon to icon component selection
    let value = props.value;
    if (!props.iconPrefixSave && value) {
      value = `ant-design:${value}`;
    }
    // update-end--author:liaozhiyang---date:20240528---for：【TV360X-136】Change button icon to icon component selection
  ...
  //update-begin---author:wangshuai ---date:20230522  for：【issues/4947】Menu edit page menu icon selection template，Display quantity per page switch is invalid，Click the icon behind the input box to clear the data.------------
  //update-begin---author:wangshuai---date:2024-05-08---for:【QQYUN-8924】Icon library selection component is not as good asvue2Convenience---
  /**
   * Click the icon to reset the number of pages
   */
  ...
  /**
   * Icon pop-up window confirms event
   */
  function handleOk() {
