    ...
    <template #overlay>
      <a-menu :class="[`${prefixCls}-menu`]" :selectedKeys="selectedKeys">
        <template v-for="item in dropMenuList" :key="`${item.event}`">
          ...
          >
            <!-- update-begin--author:liaozhiyang---date:20231110---for：【issues/839】BasicTableWhen the more operation buttons of the table are disabled, you can also click to pop up the bubble box. -->
            <a-popconfirm :disabled="item.disabled" v-if="popconfirm && item.popConfirm" v-bind="getPopConfirmAttrs(item.popConfirm)">
              <!-- update-end--author:liaozhiyang---date:20231110---for：【issues/839】BasicTableWhen the more operation buttons of the table are disabled, you can also click to pop up the bubble box. -->
...
<style lang="less">
  ...
  .@{prefix-cls} {
    // update-begin--author:sunjianlei---date:20220322---for: 【VUEN-180】More drop down menus，It only works when you click on the word，Nothing will happen if you click on an empty space，Bad experience
    ...
    // update-end--author:sunjianlei---date:20220322---for: 【VUEN-180】More drop down menus，It only works when you click on the word，Nothing will happen if you click on an empty space，Bad experience
  }
</style>
