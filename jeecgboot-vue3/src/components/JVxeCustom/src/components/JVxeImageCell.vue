<template>
  <div>
    <template v-if="hasFile" v-for="(file, fileKey) of [innerFile || {}]" :key="fileKey">
      <div class="j-vxe-image-list">
        <template v-if="!file || !(file['url'] || file['path'] || file['message'])">
          <a-tooltip :title="'please wait: ' + JSON.stringify(file) + (file['url'] || file['path'] || file['message'])">
            <LoadingOutlined />
          </a-tooltip>
        </template>
        <template v-else-if="file['path']">
          <template v-for="src of imgList">
            <img class="j-vxe-image" :src="src" alt="Picture error" @[clickEvent]="handleMoreOperation" @click="handlePreview" />
          </template>
        </template>
        <a-tooltip v-else :title="file.message || 'Upload failed'" @[clickEvent]="handleClickShowImageError">
    ...
    <div class="j-vxe-image-upload" v-if="!cellProps.disabledTable">
      ...
      >
        <a-button v-if="!hasFile" preIcon="ant-design:upload">{{ originColumn.btnText || 'Upload pictures' }}</a-button>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props: JVxeComponent.Props) {
      ...
      // update-begin--author:liaozhiyang---date:20240604---for：【TV360X-470】jVxetableUpload pictures组件限制类型
      const acceptFileType = 'image/*';
      // update-end--author:liaozhiyang---date:20240604---for：【TV360X-470】jVxetableUpload pictures组件限制类型
      // update-begin--author:liaozhiyang---date:20240105---for：【issues/953】onlineSubtablevxe-tableClick to upload the detailed image of the display form.
      const clickEvent = computed(() => {
        return unref(setup.cellProps).disabled ? null : 'click';
      });
      // update-end--author:liaozhiyang---date:20240105---for：【issues/953】onlineSubtablevxe-tableClick to upload the detailed image of the display form.
      ...
      // Pop-up upload error details
      function handleClickShowImageError() {
        let file = innerFile.value || null;
        if (file && file['message']) {
          createErrorModal({
            title: 'Upload error',
            content: 'error message：' + file['message'],
            maskClosable: true,
          });
        }
      }
      // update-begin--author:liaozhiyang---date:20240523---for：【TV360X-121】jvxetableFile component disabled state(Details)Downloadable below
      const handlePreview = () => {
        if (unref(setup.cellProps).disabled) {
          createImgPreview({ imageList: imgList.value });
        }
      };
      // update-end--author:liaozhiyang---date:20240523---for：【TV360X-121】jvxetableFile component disabled state(Details)Downloadable below
      return {
        ...setup,
        imgList,
        maxCount,
