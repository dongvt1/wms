<template>
  <div :class="{'comment-active': commentActive}" class="comment-main" @click="handleClickBlank">
    <textarea ref="commentRef" v-model="myComment" @keyup.enter="sendComment" @input="handleCommentChange" @blur="handleBlur" class="comment-content" :rows="3" placeholder="Please enter your comment，Can@member"></textarea>
    ...
    <div class="comment-buttons" v-if="commentActive">
      <div style="cursor: pointer">
        ...
        <span title="expression" style="display: inline-block">
      ...
      <div v-if="commentActive">
        ...
        <a-button type="primary" @click="sendComment" :loading="buttonLoading" :disabled="disabledButton">hair deliver</a-button>
      </div>
    </div>
...
<script lang="ts">
  ...
  const optionsName = {
    categories: {
      recent: 'most commonly used',
      smileys: 'expressionchoose',
      people: 'figure&Body',
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      function sendComment(e) {
        // update-begin--author:liaozhiyang---date:20240618---for：【TV360X-932】Comment with line break
        const keyCode = e.keyCode || e.which;
        if (keyCode == 13 && e.shiftKey) {
          return;
        }
        // update-end--author:liaozhiyang---date:20240618---for：【TV360X-932】Comment with line break
      ...
      function setValue(options) {
        ...
        if (options && options.length > 0) {
          ...
          if (realname && username) {
            ...
            // update-begin--author:liaozhiyang---date:20240726---for：【TV360X-929】choose@user，should be inserted at the cursor position
            if (!temp) {
              myComment.value = '@' + str + ' ';
            } else {
            ...
            //update-begin---author:wangshuai---date:2024-01-22---for:【QQYUN-8002】Finished selecting people，The mouse should be moved to the back and a space should be added in front---
            showHtml.value = false;
            commentRef.value.focus();
            commentActive.value = true;
            //update-end---author:wangshuai---date:2024-01-22---for:【QQYUN-8002】Finished selecting people，The mouse should be moved to the back and a space should be added in front---
          }
        }
        closeModal();        
      }
      // update-begin--author:liaozhiyang---date:20240724---for：【TV360X-927】@只有在输入时弹出user弹窗，It shouldn't pop up when deleting
      ...
      //   }
      // );
      // update-end--author:liaozhiyang---date:20240724---for：【TV360X-927】@只有在输入时弹出user弹窗，It shouldn't pop up when deleting
      ...
      function showEmoji(e) {
        ...
        // update-begin--author:liaozhiyang---date:20240603---for：【TV360X-931】评论expression插入光标位置
        const index = commentRef.value?.selectionStart ?? temp.length;
        // myComment.value = temp + str;
        const startStr = temp.substring(0, index);
        const endStr = temp.substring(index);
        myComment.value = startStr + str + endStr;
        // update-end--author:liaozhiyang---date:20240603---for：【TV360X-931】评论expression插入光标位置
      ...
      const commentHtml = computed(() => {
        let temp = myComment.value;
        if (!temp) {
          return 'Please enter your comment，Can@member';
        }
        return getHtml(temp);
      });
      ...
      function handleBlur() {
        showHtml.value = true;
        // update-begin--author:liaozhiyang---date:20240724---for：Solve the problem of inconsistent scrolling positions when multiple lines gain focus and lose focus
        setTimeout(() => {
          commentContentRef.value!.scrollTop = commentRef.value.scrollTop;
        }, 0);
        // update-end--author:liaozhiyang---date:20240724---for：Solve the problem of inconsistent scrolling positions when multiple lines gain focus and lose focus
      }
...
<style lang="less">
  ...
  .comment-content {
    ...
    border: solid 0px;
    outline: none;
    // update-begin--author:liaozhiyang---date:20240724---for：【TV360X-933】After dragging the comment box, the bottom comment list is partially covered
    resize: none;
    // update-end--author:liaozhiyang---date:20240724---for：【TV360X-933】After dragging the comment box, the bottom comment list is partially covered
    .emoji-item {
