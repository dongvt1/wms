  ...
  <div>
      ...
      <template #message>
        <span class="j-icon">
          <a-upload multiple v-model:file-list="selectFileList" :showUploadList="false" :before-upload="beforeUpload">
            <span class="inner-button"><upload-outlined />upload</span>
          </a-upload>
        </span>
      </template>
    </a-alert>
    ...
    <div class="selected-file-warp" v-if="selectFileList && selectFileList.length > 0">
      ...
      <div style="margin-bottom: 24px; margin-top: 18px; text-align: right">
        <a-button @click="quxiao">Cancel</a-button>
        <a-button type="primary" style="margin-left: 10px" @click="queding" :loading="buttonLoading">Sure</a-button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    setup(props) {
      ...
      watchEffect(() => {
        // switch every timetab都会刷新document列表--- VUEN-1884 评论里upload的picture未在document中显示
        if(props.datetime){
      ...
      function onSelectFileOk(temp) {
        // update-begin--author:liaozhiyang---date:20240603---for：【TV360X-935】从知识库选择document判断下是否没选
        if (temp.length === 0) return;
        // update-end--author:liaozhiyang---date:20240603---for：【TV360X-935】从知识库选择document判断下是否没选
        let arr = selectFileList.value;
        // -update-begin--author:liaozhiyang---date:20240614---for：【TV360X-938】知识库document选择支持多选
        temp.forEach((item) => {
          item.exist = true;
        });
        selectFileList.value = [...arr, ...temp];
        // -update-end--author:liaozhiyang---date:20240614---for：【TV360X-938】知识库document选择支持多选
      }
