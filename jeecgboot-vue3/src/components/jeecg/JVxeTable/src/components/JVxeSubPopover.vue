  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function open(event, level = 0) {
        if (level > 3) {
          createMessage.error('打开Subtable失败');
          console.warn('【JVxeSubPopover】打开Subtable失败');
          return;
        }
        ...
        // Click on the drag sort column，No processing
        ...
        if (parentElem && tr) {
          ...
          let num = ++level;
          console.warn('【JVxeSubPopover】table or tr Get失败，In progress ' + num + 'retries', {
            event,
            table: parentElem,
            tr,
          });
