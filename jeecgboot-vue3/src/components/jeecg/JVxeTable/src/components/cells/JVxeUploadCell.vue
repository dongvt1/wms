<template>
  <div>
    <template v-if="hasFile" v-for="(file, fileKey) of [innerFile || {}]" :key="fileKey">
      <a-input :readOnly="true" :value="file.name">
        <template #addonBefore style="width: 30px">
          ...
          <a-tooltip v-else-if="file.status === 'done'" title="Upload completed">
            ...
            <template #overlay>
              <a-menu>
                <a-menu-item v-if="originColumn.allowDownload !== false" @click="handleClickDownloadFile">
                  <span><Icon icon="ant-design:download-outlined" />&nbsp;download</span>
                </a-menu-item>
                <a-menu-item v-if="originColumn.allowRemove !== false" @click="handleClickDeleteFile">
                  <span><Icon icon="ant-design:delete-outlined" />&nbsp;delete</span>
                </a-menu-item>
              </a-menu>
            </template>
          </Dropdown>
    ...
    >
      <a-button preIcon="ant-design:upload-outlined">{{ originColumn.btnText || 'Click to upload' }}</a-button>
    </a-upload>
  </div>
</template>
