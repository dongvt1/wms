<template>
  <div class="slide-wrap">
    <div class="header">
      ...
      <div class="header-name">{{ appData.name || 'AI Assistant' }}</div>
    </div>
    <div class="createArea">
      <a-button type="dashed" @click="handleCreate">New Chat</a-button>
    </div>
...
<script setup lang="ts">
  ...
  const handleCreate = () => {
    const uuid = getUuid();
    props.dataSource.history.unshift({ title: 'New Chat', id: uuid, isEdit: false, disabled: true });
    // Create the first one(Need to be highlighted)
    props.dataSource.active = uuid;
    emit('click', "New Chat", 0);
  };
  // Switch chat
  ...
  function handleDel(data) {
    ...
    if (findIndex != -1) {
      ...
      // deleteof是当前activeof，activemove forward，The front is gone, move back。
    ...
    //update-begin---author:wangshuai---date:2025-03-12---for:【QQYUN-11560】New chat内容为空，无法delete---
    if(data.disabled){
      return;
    }
    //update-end---author:wangshuai---date:2025-03-12---for:【QQYUN-11560】New chat内容为空，无法delete---
    defHttp.delete({
