...
<script>
  ...
  export default {
    ...
    // listener
    watch: {
      // monitortable1 upper left【main table】选择的number据发生了变化
      ['table1.lastRow']() {
        this.loadTable2Data();
      },
      // monitortable3 upper right【main table】选择的number据发生了变化
      ['table3.lastRow']() {
        this.loadTable4Data();
      },
    },
    ...
    methods: {
      // loadtable1upper left【main table】的number据
      loadTable1Data() {
        // 封装Query条件
        let formData = {
      ...
      // whentable1upper left【main table】分页参number变化时触发的事件
      ...
      // table1upper left【main table】whenselected row变化时触发的事件
      handleTable1SelectRowChange(event) {
        this.handleTableSelectRowChange(this.table1, event);
      },

      // loadtable2lower left【Subtable】的number据，根据main table的id进行Query
      loadTable2Data() {
      ...
      // whentable2lower left【Subtable】分页参number变化时触发的事件
      ...
      // table3upper right【main table】whenselected row变化时触发的事件
      handleTable3SelectRowChange(event) {
        this.handleTableSelectRowChange(this.table3, event);
      },

      // loadtable4lower right【Subtable】的number据，根据main table的id进行Query
      loadTable4Data() {
        let parentIds = [];
        // 如果main table没有选择，则不Query
      ...
      // whentable4lower right【Subtable】分页参number变化时触发的事件
