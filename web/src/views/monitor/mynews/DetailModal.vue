<template>
  ...
  >
    ...
    <a-card class="daily-article">
      <a-card-meta :title="content.titile">
        <template #description>
          <div class="article-desc">
            <span>Posted by：{{ content.sender }}</span>
            <span>Release time：{{ content.sendTime }}</span>
            <span v-if="content.visitsNum">
              <a-tooltip placement="top" title="Visits" :autoAdjustOverflow="true">
      ...
      <div>
        <a-button v-if="hasHref" @click="jumpToHandlePage">Go to apply<ArrowRightOutlined /></a-button>
      </div>
    </a-card>
...
<script lang="ts" setup>
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      if (data.record.msgContent) {
        //update-begin---author:wangshuai---date:2023-11-15---for:【QQYUN-7049】3.6.0Version Rich text messages posted in notification announcements，View in my messages no style---
        data.record.msgContent = xss(data.record.msgContent, options);
        //update-end---author:wangshuai---date:2023-11-15---for:【QQYUN-7049】3.6.0Version Rich text messages posted in notification announcements，View in my messages no style---
      }
      //update-end-author:taoyan date:2022-7-14 for: VUEN-1702 【Forbidden questions】sqlinjection vulnerability

      //update-begin-author:liusq---date:2025-06-17--for: [QQYUN-12521]Notifications and announcements increase visits
      if (!data.record?.busId) {
        await addVisitsNum({ id: data.record.id });
      }
      //update-end-author:liusq---date:2025-06-17--for: [QQYUN-12521]Notifications and announcements increase visits
  ...
  //View message details and jump to
  ...
  function jumpToHandlePage() {
    ...
    if (temp.busId) {
      ...
      try {
        ...
        console.log('Parameter parsing exception', e);
      }
    ...
    printFrame.onload = function () {
      ...
      if (imagesToLoad === 0) {
        ...
        Array.from(images).forEach((img) => {
          ...
          // Handle images that may have been cached
