<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="isUpdate ? 'Sửa BOM' : 'Thêm BOM'" width="820px"
    @ok="handleSubmit">
    ...
    <a-table :dataSource="bomItems" :columns="itemColumns" :pagination="false" size="small" bordered class="mb-2">
      <template #bodyCell="{ column, record, index }">
        <template v-if="column.key === 'materialId'">
          <a-select v-model:value="record.materialId" show-search :options="productOptions"
            :filter-option="filterOption" placeholder="Chọn NVL" style="width: 100%" />
        </template>
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  try {
    ...
    updateSchema([{
      ...
      componentProps: {
        options: opts,
        disabled: !!data?.defaultProductId && !data?.isUpdate,
      },
    }]);
  } catch (e) { }

  // Pre-fill productId nếu mở từ màn sản phẩm
  if (data?.defaultProductId && !data?.isUpdate) {
    setFieldsValue({ productId: data.defaultProductId });
  }
