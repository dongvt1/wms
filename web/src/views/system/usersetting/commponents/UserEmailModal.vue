...
<BasicModal v-bind="$attrs" @register="registerModal" width="500px"  :title="title" :showCancelBtn="false" :showOkBtn="false">
  <a-form class="antd-modal-form" ref="formRef" :model="formState" :rules="validatorRules">
    <a-form-item name="email">
      <a-input size="large" v-model:value="formState.email" placeholder="Please enter your email" />
    </a-form-item>
    <a-form-item>
...
const validatorRules: Record<string, Rule[]> = {
  email: [{...rules.duplicateCheckRule("sys_user",'email',formState,{ label:'Mail' })[0]},{ required: true, type: 'email', message: 'Mail格式不正确' }],
};
...
async function updateEmail() {
  ...
  userEdit(formState).then((res) =>{
    if(res.success){
      createMessage.success("修改Mail成功")
      emit("success")
      closeModal();
    }else{
