<template>
    <BasicModal v-bind="$attrs" @register="registerModal" :title="isUpdate ? 'Sửa phiếu IQC' : 'Tạo phiếu kiểm tra IQC'"
...
<script lang="ts" name="iqc-inspection-modal" setup>
...
const [registerForm, { setFieldsValue, resetFields, validate, updateSchema }] = useForm({
    ...
    schemas: [
        ...
        {
            field: 'productId', label: 'Sản phẩm / NVL', component: 'Select', required: true,
            componentProps: { options: [], showSearch: true, placeholder: 'Chọn sản phẩm' }
        },
        {
            field: 'supplierId', label: 'Nhà cung cấp', component: 'Input',
            helpMessage: 'ID nhà cung cấp'
        },
        {
            field: 'templateId', label: 'Mẫu checklist', component: 'Select',
            helpMessage: 'Chọn mẫu để tự load tiêu chí',
            componentProps: { options: [], showSearch: true, placeholder: 'Chọn mẫu checklist IQC' }
        },
        {
            field: 'quantityReceived', label: 'SL nhận', component: 'InputNumber', required: true,
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    // Load IQC templates
    try {
        const templates: any = await qmsChecklistApi.listActive('iqc');
    ...
    if (unref(isUpdate) && data.record) {
        ...
        try {
            const res: any = await iqcApi.getResults(data.record.id);
...
async function handleSubmit() {
    try {
        ...
        if (unref(isUpdate)) {
            await iqcApi.edit(payload);
        } else {
            await iqcApi.add(payload);
        }
        emit('success');
        closeModal();
    } finally {
