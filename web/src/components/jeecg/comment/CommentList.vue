      ...
      <template #renderItem="{ item }">
        <a-list-item style="padding-left: 10px; flex-direction: column" @click="handleClickItem">
          <a-comment>
            ...
            <template #actions>
              <span @click="showReply(item)">reply</span>

              <Popconfirm title="Are you sure to delete?？" @confirm="deleteComment(item)">
                <span>delete</span>
              </Popconfirm>
            </template>
...
<script>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      // Avatar not obtained First two digits of username
      ...
      function getAuthor(item) {
        if (item.toUser) {
          return item.fromUserId_dictText + ' reply ' + item.fromUserId_dictText;
        } else {
      ...
      async function loadData() {
        ...
        if (!data || !data.records || data.records.length == 0) {
          ...
          // update-begin--author:liaozhiyang---date:20240521---for：【TV360X-18】The scroll bar automatically bottoms out after commenting
          // Number.MAX_SAFE_INTEGER Firefox is not compatible and changed to 10e4
          nextTick(() => {
            listRef.value && listRef.value.$el && (listRef.value.$el.scrollTop = 10e5);
          });
          // update-end--author:liaozhiyang---date:20240521---for：【TV360X-18】The scroll bar automatically bottoms out after commenting
        }
      }
        ...
        if(v==true){
          if(!item.commentId_dictText){
            ...
            if(data.success == true){
              ...
              item.commentId_dictText='该Comment已被delete';
            }
          }
        }
      }
      // update-begin--author:liaozhiyang---date:20240618---for：【TV360X-932】Comment加上换行
      const lineFeed = (content) => {
        return content.replace(/\n/g, '<br>');
      };
      // update-end--author:liaozhiyang---date:20240618---for：【TV360X-932】Comment加上换行
