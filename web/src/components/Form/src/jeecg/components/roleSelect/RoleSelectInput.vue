<template>
  <div>
    <div @click="showModal" :class="disabled ? 'select-input disabled-select' : 'select-input'">
      ...
      <span v-else style="height: 30px; line-height: 30px; display: inline-block; margin-left: 7px; color: #bfbfbf">Please select</span>
    ...
    <RoleSelectModal :appId="currentAppId" :multi="multi" :getContainer="getContainer" title="Select organizational role" @register="registerRoleModal" @selected="onSelected" />
  </div>
</template>
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      function onSelectedChange() {
        ...
        console.log('Select data', str);
      }

      function onSelected(_v, values) {
        console.log('Character selection completed：', values);
        loading.value = false;
      ...
      // Currently only a state used for data reloading
      ...
      async function getRoleList(ids) {
        ...
        // Special condition handling（Because the background entity isroleCode，So make a compromise，cannot be changed directly，something will go wrong）
        if (props.store === 'code') {
          params.roleCode = ids;
        }
        selectedList.value = [];
        ...
        if (data.success) {
          ...
          if (records && records.length > 0) {
            ...
            // according to ids the order of arr Sort
            if (props.store === 'code') {
