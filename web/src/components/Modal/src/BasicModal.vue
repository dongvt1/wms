<template>
  <Modal v-bind="getBindValue" @cancel="handleCancel">
    ...
    <!-- update-begin-author:taoyan date:2022-7-18 for:  modalPop-up window Support comments slot -->
    ...
    <!-- update-end-author:taoyan date:2022-7-18 for:  modalPop-up window Support comments slot -->
  ...
  export default defineComponent({
    ...
    setup(props, { emit, attrs , slots}) {
      ...
      const getMergeProps = computed((): Recordable => {
        ...
        // update-begin--author:liaozhiyang---date:20240326---for：【QQYUN-8643】Pop-up window移动端Pop-up window统一全屏
        if (getIsMobile.value) {
          result.canFullscreen = false;
          result.defaultFullscreen = true;
        }
        // update-end--author:liaozhiyang---date:20240326---for：【QQYUN-8643】Pop-up window移动端Pop-up window统一全屏
        return result;
      });
        //update-begin-author:liusq date:2023-05-25 for:【issues/4856】ModalControl settings :title = null invalid
        //Whether the title is not set
        const isNoTitle = computed(() => {
            //The title is empty and contains no title slot
            return !unref(getMergeProps).title && !slots.title;
        });
        //update-end-author:liusq date:2023-05-25 for:【issues/4856】ModalControl settings :title = null invalid
      ...
      /**
       * @description: set upmodalparameter
       */
      ...
      //update-begin-author:taoyan date:2022-7-18 for: modalSupport comments slot
      ...
      function handleComment(flag){
        ...
        // update-begin--author:liaozhiyang---date:20240528---for：【TV360X-485】开启评论之后Pop-up window按钮居右隔一个评论的距离
        emit('comment-open', commentSpan.value === 0, commentSpan.value);
        // update-end--author:liaozhiyang---date:20240528---for：【TV360X-485】开启评论之后Pop-up window按钮居右隔一个评论的距离
      }
      //update-end-author:taoyan date:2022-7-18 for: modalSupport comments slot
...
<style lang="less">
  /*update-begin-author:taoyan date:2022-7-27 for:modalComment area style*/
  .jeecg-comment-outer {
  ...
  .jeecg-modal-content{
    >.scroll-container{
      //update-begin---author:wangshuai---date:2023-12-05---for:【QQYUN-7297】表单讨论Pop-up window放大按钮时只显示一部分---
      padding: 6px;
      //update-end---author:wangshuai---date:2023-12-05---for:【QQYUN-7297】表单讨论Pop-up window放大按钮时只显示一部分---
    }
  }
  /*update-end-author:taoyan date:2022-7-27 for:modalComment area style*/

  // wrapperset to100%，兼容之前写过的Pop-up window自定义样式
  .jeecg-modal-wrapper,
  .jeecg-modal-content {
    height: 100%;
  }
